...
class MC(Mqtt_client):
    ...
    def on_message(self, client, userdata, msg):
            ...
            if 'motion' in topic:
                mainwin.statusDock.motion_dist.setText(check(m_decode.split('Temperature: ')[1]))
...
class StatusDock(QDockWidget):
    ...
    def __init__(self,mc):
        QDockWidget.__init__(self)        
        self.mc = mc
        self.motion_dist = QLabel()
        self.motion_dist.setText("80")
        self.motion_dist.setStyleSheet("color: red")
        self.wifi = QLabel()
        ...
        formLayot=QFormLayout()
        formLayot.addRow("Motion:", self.motion_dist)
...
class TempDock(QDockWidget):
    ...
    def __init__(self,mc):
        ...
        formLayot=QFormLayout()       
        formLayot.addRow("Motion detector",self.tBoiler)
    ...
    def tb_selectionchange(self,i):
        ...
        if "ON" in self.tBoiler.currentText():
            self.tBoiler.setStyleSheet("color: green")
            # self.mc.publish_to('pr/MGCar/Motion/sub','Set temperature to: ')
