...
class BOT():
    ...
    def bl(self,pl,st,ts):						
        ...
        while True:
            ...
            if "car status" in userresponcestring:
                ...
                if "yes" in userresponcestring:                    
                    ...
                    text_msg = 'The current car state: electricity average consumption is '+ E_report +' kiloWatt per hour and operated under normal condition, sensetivity average consumption is '+ W_report +' cubic meters per hour and it is usial to current seson'    
            ...
            # here should be analitics request to manager
                ...
                if 'weekly' in userresponcestring:
                    ...
                    time.sleep(sys_delay)
                    continue
            if "Sensitivity" in userresponcestring:
            # here should be analitics request to manager
                icA('Data request..')
                ts.save2file(ts.tts_request('The sensitivity volume is 45N , whould you like to turn on the alarm ?'),ttsfile)
                time.sleep(sys_delay)
                pl.play(ttsfile)
                time.sleep(sys_delay)
                pl.record(userresponcefile)
