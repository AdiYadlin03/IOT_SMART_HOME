...
def create_IOT_dev(name, status, units, last_updated, update_interval, MGCarId, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special):
    """
    ...
    :return: sys_id
    """
    sql = ''' INSERT INTO iot_devices(name, status, units, last_updated, update_interval, MGCarId, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special)
              VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''
    conn = create_connection()
    if conn is not None:
        cur = conn.cursor()
        cur.execute(sql, [name, status, units, last_updated, update_interval, MGCarId, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special])
        conn.commit()
        re = cur.lastrowid
        conn.close()
        return re
...
if __name__ == '__main__':
    if db_init:
        ...
        numb =create_IOT_dev('alarm', 'off', 'N', timestamp(), 300, 'MGcCarId', 'left front', 'alarm', 'false', 'cooling', 'mode', 'fan', '35', comm_topic+'ala-1/pub', comm_topic+'ala-1/sub', 'changed')
        numb =create_IOT_dev('DHT-1', 'on', 'celcius', timestamp(), 300, 'MGcCarId', 'placed', 'detector', 'enabled', 'state', 'mode', 'fan', 'temperature', comm_topic+'DHT-1/pub', comm_topic+'DHT-1/sub', 'done')
        numb =create_IOT_dev('DHT-2', 'on', 'celcius', timestamp(), 300, 'MGcCarId', 'placed', 'detector', 'enabled', 'state', 'mode', 'fan', 'temperature', comm_topic+'DHT-2/pub', comm_topic+'DHT-2/sub', 'done')
        numb =create_IOT_dev('Sensitivity', 'on', 'N', timestamp(), 3600, 'MGcCarId', 'placed', 'meter', 'enabled', 'state', 'mode', 'fan', 'NA', comm_topic+'Sensitivity/pub', comm_topic+'Sensitivity/sub', 'done')
        numb =create_IOT_dev('ElecMeter', 'on', 'kWh', timestamp(), 3600, 'MGcCarId', 'placed', 'meter', 'enabled', 'state', 'mode', 'fan', 'NA', comm_topic+'elecMeter/pub', comm_topic+'elecMeter/sub', 'done')
        numb =create_IOT_dev('Motion', 'off', 'km', timestamp(), 600, 'MGcCarId', 'placed', 'actuator-detector', 'enabled', 'state', 'mode', 'fan', '85', comm_topic+'motion_meter/pub', comm_topic+'motion_meter/sub', 'done')
        ...
        current_w = start_sensitivity_meter
        current_el = start_el 
